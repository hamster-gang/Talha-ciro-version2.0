...
class _ReportIncidentScreenState extends State<ReportIncidentScreen>
  ...
  static const _types = [
    'Flooding', 'Fire', 'Heatwave', 'Infrastructure Failure',
    'Medical Emergency', 'Road Accident', 'Power Outage', 'Other',
  ];
  ...
  @override
  void dispose() {
    _descCtrl.dispose(); _locCtrl.dispose(); _nameCtrl.dispose();
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _lng = lng;
          _gpsConfirmed = true;
          _locCtrl.text = address ?? '${lat.toStringAsFixed(5)}, ${lng.toStringAsFixed(5)}';
          _gpsLoading = false;
        });
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Location access denied. Please type your location manually.'),
  ...
  Future<void> _submit() async {
    ...
    try {
      final body = json.encode({
        ...
        'reporter_name': _nameCtrl.text.trim().isEmpty ? null : _nameCtrl.text.trim(),
        'lat': _lat,
        'lng': _lng,
      });
  ...
  Widget _buildSuccessView() {
    return Center(child: ScaleTransition(scale: _successScale, child: SingleChildScrollView(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const Text('CIRO agents have received your report and will prioritize response.',
        ...
        Container(
          ...
          child: Column(children: [
            const Text('YOUR TRACKING ID', style: TextStyle(color: Colors.white38, fontSize: 11, letterSpacing: 2)),
            const SizedBox(height: 6),
            Text(_trackingId ?? '—',
              style: const TextStyle(color: Color(0xFF42A5F5), fontSize: 22, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 6),
            const Text('Use this ID in the Safety Center to track your report.',
              textAlign: TextAlign.center,
        ...
        SizedBox(width: double.infinity, child: ElevatedButton(
          ...
          child: const Text('Back to Safety Center', style: TextStyle(fontWeight: FontWeight.bold)),
        )),
      ]),
    )));
  }
  ...
  Widget _buildForm() {
    return Form(key: _formKey, child: ListView(padding: const EdgeInsets.all(24), children: [
      const Text('INCIDENT TYPE', style: TextStyle(color: Colors.white38, fontSize: 11, letterSpacing: 2, fontWeight: FontWeight.bold)),
      const SizedBox(height: 12),
      Wrap(spacing: 10, runSpacing: 10, children: _types.map((type) {
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: selected ? const Color(0xFF1565C0) : Colors.white.withOpacity(0.06),
              borderRadius: BorderRadius.circular(30),
              border: Border.all(color: selected ? const Color(0xFF42A5F5) : Colors.white12),
              boxShadow: selected ? [const BoxShadow(color: Color(0x331565C0), blurRadius: 8)] : [],
            ),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(_typeIcons[type] ?? Icons.report_problem,
                color: selected ? Colors.white : Colors.white54, size: 16),
              const SizedBox(width: 6),
              Text(type, style: TextStyle(color: selected ? Colors.white : Colors.white60, fontSize: 13, fontWeight: selected ? FontWeight.bold : FontWeight.normal)),
            ]),
          ),
        );
      }).toList()),
      const SizedBox(height: 28),
      const Text('DESCRIPTION', style: TextStyle(color: Colors.white38, fontSize: 11, letterSpacing: 2, fontWeight: FontWeight.bold)),
      const SizedBox(height: 10),
      TextFormField(
        controller: _descCtrl,
        style: const TextStyle(color: Colors.white),
        maxLines: 4,
        decoration: _inputDecoration('Describe what you see... (e.g. "Street flooded near G-10 Markaz")'),
        validator: (v) => (v == null || v.trim().length < 10) ? 'Please describe the incident (min 10 chars)' : null,
      ),
      const SizedBox(height: 20),
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        const Text('LOCATION', style: TextStyle(color: Colors.white38, fontSize: 11, letterSpacing: 2, fontWeight: FontWeight.bold)),
        if (_gpsConfirmed)
          Row(children: [
            Icon(Icons.gps_fixed, color: Colors.greenAccent, size: 12),
            const SizedBox(width: 4),
            const Text('GPS Confirmed', style: TextStyle(color: Colors.greenAccent, fontSize: 10)),
          ]),
      ]),
      const SizedBox(height: 10),
      Row(
          ...
          Tooltip(
            ...
            child: InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: _gpsConfirmed ? Colors.green.withOpacity(0.15) : Colors.blueAccent.withOpacity(0.1),
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: _gpsConfirmed ? Colors.green.withOpacity(0.4) : Colors.blueAccent.withOpacity(0.3)),
                ),
                child: _gpsLoading
                  ...
                  : Icon(
                      ...
                      color: _gpsConfirmed ? Colors.greenAccent : Colors.blueAccent,
                      size: 24,
                    ),
              ),
            ),
      ...
      const SizedBox(height: 20),
      const Text('YOUR NAME (OPTIONAL)', style: TextStyle(color: Colors.white38, fontSize: 11, letterSpacing: 2, fontWeight: FontWeight.bold)),
      const SizedBox(height: 10),
      TextFormField(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF0D1B3E), foregroundColor: Colors.white,
